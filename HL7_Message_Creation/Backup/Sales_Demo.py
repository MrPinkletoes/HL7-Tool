...
def get_lines():
    ...
    with open('names_m.csv', 'r') as csvfile:
    ...
    with open('names_f.csv', 'r') as csvfile:
    ...
    with open('sampletype.csv', 'r') as csvfile:
    ...
    with open('locations_code.csv', 'r') as csvfile:
    ...
    string_7 = random.randrange(['namem.name', 'namef.name'])
    ...
    line2 = "\nPID|1||%s^^^^HOSPITALNO~%s^^^^NHSNO||%s^%s||%s0000|%s|||||||||||||| " % (
        hosp_no, nhs_no, surname, string_7, dob, sex)
    line3 = "\nPV1|1||%s|||||||||||||||||||||||||||||||||||||||||||||||| " % location
...
files = int(input("How many files would you like?:"))
print(files)
...
while os.path.exists("S360_%s.xml" % i):
    ...
    if i == files:
        print("Complete, Now run the Move_files_to_S360.bat")
        i += 1
    else:
        i += 1
        f = open('S360_%s.xml' % i, "w+")
